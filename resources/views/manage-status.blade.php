...
<div class="container-fluid mt-4">
    ...
    <a href="{{ route('addStatus') }}" class="btn btn-primary mb-3 float-left" data-toggle="tooltip" data-placement="left" title="Add New Status"><i class="fa-solid fa-plus"></i></a>
        ...
        <tbody>
            ...
            <tr data-status="{{ is_null($status->deleted_at) ? 'active' : 'inactive' }}">
                ...
                <td class="text-center">
                    <input type="hidden" class="status-value" value="{{ $status->is_active }}">



                   
                    <a href="{{ route('editStatus', $status->id) }}" class="btn btn-primary btn-edit">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                   

                    <label class="switch btn-toggle">
                        <input type="checkbox" {{ is_null($status->deleted_at) ? 'checked' : '' }} data-id="{{ $status->id }}">
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '.switch input', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `You want to ${isActive ? 'activate' : 'deactivate'} this status?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/status/' + statusId + '/update-status',
                        type: 'PATCH',
                        headers: {
