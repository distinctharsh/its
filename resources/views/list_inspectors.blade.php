                ...
                @else
                    ...
                    <tr data-status="{{ is_null($row->v_deleted_at) ? 'active' : 'inactive' }}" data-join-date="{{ $row->arrival_datetime ? date('Y-m-d') : '' }}">
                        ...
                        <td class="text-center">
                            @php
                                try {
                                    $tlName = Crypt::decryptString($row->tl_name);
                        ...
                        <style>
                            ...
                            .no-bullets li {
                                margin-bottom: 5px; /* Maintain spacing between list items */
                            }
                        </style>
                        ...
                        <td class="text-center">
                            @php
                                ...
                                $rawInspectors = $row->list_of_inspectors ?? '[]'; // Default to empty JSON if null

                                // Decode JSON safely
                                $decodedOnce = json_decode($rawInspectors, true);

                                // If still a string, decode again (handles double-encoding issues)
                                $inspectorsList = is_string($decodedOnce) ? json_decode($decodedOnce, true) : $decodedOnce;

                                // Ensure it's always an array
                                if (!is_array($inspectorsList)) {
                                    $inspectorsList = [];
                                }

                                // Convert all IDs to integers
                                $inspectorsList = array_map('intval', $inspectorsList);

                                // Fetch inspector names from the database
                                $inspectorNames = \DB::table('inspectors')
                                    ->whereIn('id', $inspectorsList)
                                    ->pluck('name')
                                    ->toArray();

                                // If names are encrypted, decrypt them
                                $inspectorNames = array_map(fn($name) => Crypt::decryptString($name), $inspectorNames);
                            @endphp

                            @if (!empty($inspectorNames))
                                <ul class="no-bullets">
                                    @foreach ($inspectorNames as $name)
                                        <li>{{ $name }}</li>
                                    @endforeach
                                </ul>
                            @else
                                No matching inspectors found
                            @endif
                        </td>
...
<script>
    ...
    function getInspectorNames(inspectorIds) {
        if (!Array.isArray(inspectorIds)) return "Invalid Format";

        return inspectorIds.map(id => {
            let inspector = allInspectors.find(inspector => inspector.id == id);
            return inspector ? inspector.name : "Unknown Inspector";
        }).join(", ");
    }

    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll("td[data-inspectors]").forEach(td => {
            let inspectorIds = JSON.parse(td.dataset.inspectors || "[]");
            td.querySelector(".inspector-names").innerText = getInspectorNames(inspectorIds);
        });
    });
</script>
