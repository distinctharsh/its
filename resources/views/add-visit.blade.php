...
<div class="modal fade" id="detailsModal" tabindex="-1" role="dialog" aria-labelledby="detailsModalLabel" aria-hidden="true" style="top: 140px;">
    ...
    <button type="submit" class="btn btn-primary" id="modalSubmitBtn">
        <span id="modalSubmitText">Add</span>
        <span id="modalSubmitSpinner" class="spinner-border spinner-border-sm d-none" role="status" aria-hidden="true"></span>
    </button>
...
<script src="{{ url('assets/vendor/datatables/select2.min.js') }}"></script>
<script>
...
function resetModal() {
    ...
    $('#category_type_id').change(function () {
        ...
        if (selectedTypeId == 2) {
            ...
            for (let i = 0; i < iterationCount; i++) {
                inspectionFields.append(inspectionCategoryTemplate([2, 3, 4]));
                appendCommonFields();
        ...
        const filteredCategories = inspectionTypes.filter(type => selectedTypeId == 2 || (selectedTypeId == 1 && [2, 3, 4].includes(type.id)));
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (faxData && faxData['fax_document']) {
                                ...
                                var faxDocumentUrl = '{{ url("storage/app/") }}' + '/' + faxData['fax_document'];
    ...
    var submitText = $('#modalSubmitText');
    var submitSpinner = $('#modalSubmitSpinner');
            ...
            if (validateForm()) {
                ...
                submitSpinner.addClass('d-none');
                        if (response.success) {
                            ...
                            var faxDocumentUrl = '{{ url("storage/app/") }}' + '/' + response.fax_document;
