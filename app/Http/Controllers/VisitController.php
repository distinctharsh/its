...
class VisitController extends Controller
{
    ...
    public function manageVisit()
    {
        ...
        return view('manage-visit', compact('visits', 'site_codes', 'inspection_phases', 'states', 'visit_categories', 'entry_exit_points'));
    }
    ...
    public function show($id)
    {
        ...
        $entry_exit_points = EntryExitPoint::whereNull('deleted_at')->get();
        return view('visit-details', compact('visit', 'inspectors', 'escortOfficers', 'states', 'siteCodes', 'entry_exit_points'));
    }
        ...
        try {
            ...
            $validated['visit_report'] = $request->hasFile('visit_report')
                ...
                if ($request->hasFile('preliminary_report')) {
                    $files = $request->file('preliminary_report');
                    if (!is_array($files)) {
                        $files = [$files]; // Ensure it's an array if only one file is uploaded
                    }
                    foreach ($files as $file) {
                        $preliminaryReportPaths[] = $file->store('visit_preliminary_reports');
                    }
                } else {
                    // If no preliminary report is uploaded, keep it as empty array or null
                    $preliminaryReportPaths = null;
                }
                ...
                if ($request->hasFile('issue_document')) {
                    ...
                    // Ensure it's an array, whether it's a single file or multiple files
                    $files = is_array($files) ? $files : [$files];

                    foreach ($files as $file) {
                        // Store the file and add the path to the array
                        $issueDocumentPaths[] = $file->store('visit_issue_documents');
                    }
                } else {
                    $issueDocumentPaths = null;
                }
                ...
                if ($request->hasFile('final_inspection_report')) {
                    ...
                    $finalInspectionReportPaths = null;
                }
            ...
            $visitSiteMapping = VisitSiteMapping::where('visit_id', $visit->id)->get(); // Get the collection

            if ($visitSiteMapping->isEmpty()) {
                Log::warning('No visit site mappings found for this visit.');
            } else {
                // Loop through the collection and delete each record
                foreach ($visitSiteMapping as $mapping) {
                    $mapping->delete(); // Delete each individual visit site mapping
                    $recordId = $mapping->id; // Get the ID of the deleted record
            ...
            for ($i = 0; $i < count($siteCodes); $i++) {
                ...
                // Store the issue_document paths in the column (as a JSON-encoded string if there are multiple files)
                if (!empty($issueDocumentPaths)) {
                    $siteMappingData['issue_document'] = json_encode($issueDocumentPaths);
                } elseif ($existingMapping) {
                    // Retain the original issue document if it exists
                    $siteMappingData['issue_document'] = $existingMapping->issue_document;
                }

                // Include preliminary report only if it's not null or empty
                if (isset($preliminaryReportPaths[$i]) && !empty($preliminaryReportPaths[$i])) {
                    ...
                    // Retain the original preliminary report if it exists
                    $siteMappingData['preliminary_report'] = $existingMapping->preliminary_report;
                }

                // Include final inspection report only if it's not null or empty
                if (isset($finalInspectionReportPaths[$i]) && !empty($finalInspectionReportPaths[$i])) {
                    ...
                    // Retain the original final inspection report if it exists
                    $siteMappingData['final_inspection_report'] = $existingMapping->final_inspection_report;
                }
